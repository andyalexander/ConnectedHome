...
void loop () {
    ...
    if (p != 0)
    {
        if (orscV2.nextPulse(p))
        {
            ...
            // just for DEBUG
            Serial.println(OregonType(DataDecoded));
            Serial.println(channel(DataDecoded));
            Serial.println(temperature(DataDecoded));
            Serial.println(humidity(DataDecoded));
            Serial.println(battery(DataDecoded));
